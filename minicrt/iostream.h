...
namespace std {
  class ofstream {
    ...
    public:
      ...
      ofstream();
      explicit ofstream(const char *filename, ofstream::openmode mode = ofstream::out);
      ~ofstream();

      ofstream &operator<<(char c);
      ofstream &operator<<(int c);
      ofstream &operator<<(const char *str);
      ofstream &operator<<(ofstream &(*)(ofstream &));
      void open(const char *filename, ofstream::openmode mode = ofstream::out);
      void close();
      ofstream &write(const char *buf, unsigned int size);
  };
